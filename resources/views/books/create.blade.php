...
@section('title', 'Tambah Buku Baru - AoBook')

@section('content')
<h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-6 transition-colors duration-300">Tambah Buku Baru</h1>
        ...
        <form action="{{ route('books.store') }}" method="POST">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="text" name="title" id="title" class="px-4 py-2 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md @error('title') border-red-500 @enderror" value="{{ old('title') }}" placeholder="Contoh: Laskar Pelangi">
                    </div>
                    @error('title')
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="number" name="year" id="year" class="px-4 py-2 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md @error('year') border-red-500 @enderror" value="{{ old('year') }}" placeholder="Contoh: 2005">
                    </div>
                     @error('year')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
